...
`cwc`、`CWC` 与 `Codex with ChatGPT` 等价。安装 Skill 后，资料查询、概念解释、
比较和讨论默认走 ChatGPT 仅查询流程；明确说“不用 cwc”可跳过。

- “NAS 是什么？”／“用 cwc 比较这两种方案”：直接聊天，按需联网查证，不要求
  工作目录、项目、插件、连接检查或配对，也不选择或确认模型档位。
- “查询这个工程的登录逻辑”：复用已有只读插件读取必要文件，只回答，不改动。
- “用 cwc 帮我修复登录问题”：走规划 → Codex 执行和测试 → ChatGPT 复核。

Codex 与 ChatGPT 之间的问题、方案和复核，以及面向用户的汇报均使用中文；代码、命令、路径和协议字段保留原样。

查询里的修复建议不会自动执行。每个工作目录复用一个插件，切换聊天无需重新
创建连接；禁止通过删除、卸载或重建插件来重连。实际授权失效时恢复原连接授权；
地址变化时仅尝试界面支持的原地更新，不支持就保留插件并说明阻塞原因。

### 可选的固定域名

默认公网地址是临时的，桥重启后可能变化。Codex 保留已有插件，仅在界面支持时原地更新地址；否则说明阻塞原因。普通查询仍可直接聊天。
